...
void KEYDBNC_Process(void) {
  ...
  /*! \todo Only debounce if you are not debouncing already */
	...
	//if(KEYDBNC_FSMdata.state == DBNC_KEY_IDLE && KEYDBNC_GetKeys()!=0) für nicht interrupt
	//obere Zeile verwenden, falls via Polling. Im main.c. einfach immer KEYDBNC_Process aufrufen
